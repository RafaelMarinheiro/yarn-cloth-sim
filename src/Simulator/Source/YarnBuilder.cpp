...
void YarnBuilder::buildBraid() {
  ...
  const Vec3e origin(0.0, 10.0, 0.0);
  
  std::vector<Vec3e> points;
  ...
  Vec3e u = (points[1] - points[0]).cross(Vec3e(1.0, 0.0, 0.0)).normalized();
  
  std::string filename = rod::resource::pathToOutput("braid.yarn");
